...
from .plot_utils import (
    print_data_table, customize, ICTS, ICHIST, ICQQ, QRETURNBAR, QRETURNVIOLIN,
...
def plot_returns_table(alpha_beta, mean_ret_quantile, mean_ret_spread_quantile):
    ...
    print("收益分析")
    print_data_table(returns_table.apply(lambda x: x.round(3)))
...
def plot_turnover_table(autocorrelation_data, quantile_turnover):
    ...
    print("换手率分析")
    print_data_table(turnover_table.apply(lambda x: x.round(3)))
    print_data_table(auto_corr.apply(lambda x: x.round(3)))
...
def plot_information_table(ic_data):
    ...
    print("IC 分析")
    print_data_table(ic_summary_table.apply(lambda x: x.round(3)).T)
...
def plot_quantile_statistics_table(factor_data):
    ...
    print("分位数统计")
    print_data_table(quantile_stats)
