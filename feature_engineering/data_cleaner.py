...
def timestamp_matching_handing(df: pd.DataFrame, code: str, standard_span: list):
    ...
    :param code:一只股票的代码
    :param standard_span:标准时间跨度，通过span函数得到
    :return: df：经过时间戳匹配后的特征因子的dataframe
    """
    temp_mindex = pd.DataFrame(df.index)
...
def missing_value_handing(factor_series: pd.Series, method='ffill', rolling_span: int = 3, knn_k: int = 3):
    ...
    if method == 'ffill':
        ...
        if rolling_span < 2:
            raise ValueError(f"The span of the rolling average must be greater than or equal to 2.")
        else:
...
def outlier_replace(df_feature: pd.DataFrame, min_value: float = 0):
    """
...
def timestamp_matching(df_feature: pd.DataFrame):
    ...
    for code, df in df_feature.groupby('code'):
        df = timestamp_matching_handing(df, str(code), standard_span)
        df = df.reset_index()
